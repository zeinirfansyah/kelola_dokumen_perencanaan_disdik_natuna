...
class ForumController extends Controller
{
    public function index(Request $request) {
        ...
        $years =Forum::select('tahun')->distinct()->get();
    ...
    public function storeDocument(Request $request)
    {
       ...
       Forum::create($document);

        return redirect()->route('forum.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Forum::find($id);
        ...
        Storage::delete("public/documents/forum/{$filename}");
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/forum' directory
            $document->storeAs('public/documents/forum', $filename);
    ...
    {
        $document = Forum::find($id);
        return view('admin.forum.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Forum::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/forum/{$currentFile}");
        }
        return redirect()->route('forum.index', ['id' => $id])->with('success', 'Forum updated successfully');
    }
}
