...
class KdpController extends Controller
{
    ...
    public function storeDocument(Request $request)
    {
       ...
       Kdp::create($document);
        ...
        return redirect()->route('kdp.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Kdp::find($id);
        ...
        Storage::delete("public/documents/kdp/{$filename}");

        return redirect()->route('kdp.index')->with('success', 'Kdp deleted successfully');
    }
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/kdp' directory
            $document->storeAs('public/documents/kdp', $filename);
    ...
    {
        $document = Kdp::find($id);
        return view('admin.kdp.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Kdp::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/kdp/{$currentFile}");
        }
        return redirect()->route('kdp.index', ['id' => $id])->with('success', 'Kdp updated successfully');
    }
}
