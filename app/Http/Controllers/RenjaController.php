...
class RenjaController extends Controller
{
    ...
    {
        return view('admin.renja.create');
    }
    ...
    public function storeDocument(Request $request)
    {
       ...
       Renja::create($document);

        return redirect()->route('renja.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Renja::find($id);
        ...
        Storage::delete("public/documents/renja/{$filename}");

        return redirect()->route('renja.index')->with('success', 'Renja deleted successfully');
    }
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/renja' directory
            $document->storeAs('public/documents/renja', $filename);
    ...
    {
        $document = Renja::find($id);
        return view('admin.renja.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Renja::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/renja/{$currentFile}");
        }
        return redirect()->route('renja.index', ['id' => $id])->with('success', 'Renja updated successfully');
    }
}
