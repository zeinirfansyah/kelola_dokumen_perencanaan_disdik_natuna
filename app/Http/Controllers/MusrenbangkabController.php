...
class MusrenbangkabController extends Controller
{
    public function index(Request $request) {
        ...
        return view('admin.musrenbangkab.index', [
            'documents' => $documents,
            'years' => $years,
        ]);
    }
    ...
    {
        return view('admin.musrenbangkab.create');
    }
    ...
    public function storeDocument(Request $request)
    {
       ...
       Musrenbangkab::create($document);

        return redirect()->route('musrenbangkab.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Musrenbangkab::find($id);
        ...
        Storage::delete("public/documents/musrenbangkab/{$filename}");

        return redirect()->route('musrenbangkab.index')->with('success', 'Musrenbangkab deleted successfully');
    }
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/musrenbangkab' directory
            $document->storeAs('public/documents/musrenbangkab', $filename);
    ...
    {
        $document = Musrenbangkab::find($id);
        return view('admin.musrenbangkab.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Musrenbangkab::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/musrenbangkab/{$currentFile}");
        }
        return redirect()->route('musrenbangkab.index', ['id' => $id])->with('success', 'Musrenbangkab updated successfully');
    }
}
