...
class PeraturanController extends Controller
{
    ...
    public function storeDocument(Request $request)
    {
       ...
       Peraturan::create($document);

        return redirect()->route('peraturan.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Peraturan::find($id);
        ...
        Storage::delete("public/documents/peraturan/{$filename}");

        return redirect()->route('peraturan.index')->with('success', 'Peraturan deleted successfully');
    }
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/peraturan' directory
            $document->storeAs('public/documents/peraturan', $filename);
    ...
    {
        $document = Peraturan::find($id);
        return view('admin.peraturan.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Peraturan::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/peraturan/{$currentFile}");
        }
        return redirect()->route('peraturan.index', ['id' => $id])->with('success', 'Peraturan updated successfully');
    }
}
