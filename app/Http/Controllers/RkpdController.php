...
class RkpdController extends Controller
{
    ...
    public function storeDocument(Request $request)
    {
       ...
       Rkpd::create($document);

        return redirect()->route('rkpd.index')->with('success', 'Document data created successfully');
    }

    public function deleteDocument($id)
    {
        $document = Rkpd::find($id);
        ...
        Storage::delete("public/documents/rkpd/{$filename}");

        return redirect()->route('rkpd.index')->with('success', 'Rkpd deleted successfully');
    }
    ...
    private function handleDocumentUpload(Request $request, $currentDocument)
    {
        ...
        if ($request->hasFile('file') && $request->file('file')->isValid()) {
            ...
            // Save the file in the 'public/documents/rkpd' directory
            $document->storeAs('public/documents/rkpd', $filename);
    ...
    {
        $document = Rkpd::find($id);
        return view('admin.rkpd.update', compact('document'));
    }
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        $document = Rkpd::where('id', $id)->update($document);
        // Delete the old file if it's different from the new one
        if ($currentFile !== $filename) {
            Storage::delete("public/documents/rkpd/{$currentFile}");
        }
        return redirect()->route('rkpd.index', ['id' => $id])->with('success', 'Rkpd updated successfully');
    }
}
